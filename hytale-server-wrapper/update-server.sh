...
# TOUJOURS déplacer les fichiers depuis Server/ vers la racine
if [ -d "$DATA_DIR/Server" ]; then
    log_info "Déplacement des fichiers depuis Server/ vers /data/..."
    
    # Copier les fichiers essentiels
    if [ -f "$DATA_DIR/Server/HytaleServer.jar" ]; then
        cp "$DATA_DIR/Server/HytaleServer.jar" "$DATA_DIR/" && log_success "HytaleServer.jar copié"
    fi
    
    if [ -f "$DATA_DIR/Server/HytaleServer.aot" ]; then
        cp "$DATA_DIR/Server/HytaleServer.aot" "$DATA_DIR/" && log_success "HytaleServer.aot copié"
    fi
    
    # Copier les autres fichiers si présents
    find "$DATA_DIR/Server" -maxdepth 1 -type f \( -name "*.jar" -o -name "*.aot" \) -exec cp {} "$DATA_DIR/" \;
    
    log_success "Fichiers déplacés vers /data/"
else
    log_warn "Pas de dossier Server/, fichiers déjà à la racine"
fi
